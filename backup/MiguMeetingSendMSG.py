...
class MiguMeetingSendMSG(object):
    ...
    def parse_message(self, message):
        if "" == message:
            ...
            if msg_type == '2':
                if __name__ != "__main__":
                    self.locust_environment.events.request_success.fire(request_type="WebSocket", name='msg_ack',
                                                                        response_time=0,
                                                                        response_length=0)
                return
            ...
            if msg_type == "0":
                ...
                if 'start_sendmsg' in msg:
                    ...
                    if __name__ != "__main__":
                        self.locust_environment.events.request_success.fire(request_type="WebSocket", name='发送消息',
                                                                            response_time=0,
                                                                            response_length=0)
    ...
    def reconnect(self, reason=None):
        if __name__ != "__main__":
            self.locust_environment.events.request_failure.fire(request_type="WebSocket", name='重新连接',
                                                                response_time=0,
    ...
    def on_open(self, state):
        logger.debug("开启连接！！")
        if __name__ != "__main__":
            self.locust_environment.events.request_success.fire(request_type="WebSocket", name='启动IM连接',
                                                                response_time=0,
                                                                response_length=0)
        ...
        class HeartbeatThread(Thread):
            ...
            def run(self):
                ...
                while True:
                    ...
                    if count >= 60:
                        msg_body = {"type": "0", "data": ["fzf_perftest "], "msgId": self.__msgId}
                        self.ws.send(json.dumps(msg_body), ABNF.OPCODE_TEXT)
                        self.__msgId = self.__msgId + 1
                        count = 0
                        logger.debug("发送消息！")
                        if __name__ != "__main__":
                    ...
                    if __name__ != "__main__":
                        self.locust_environment.events.request_success.fire(request_type="WebSocket", name='发送心跳',
                                                                        response_time=0,
                                                                        response_length=0)
                    self.event.wait(timeout=HEART_BEAT_PERIOD)
                    count = count + HEART_BEAT_PERIOD

        """发送心跳"""
        event1 = Event()
        heartbeat = HeartbeatThread(event1, self.__socket_client, self.locust_environment)
        heartbeat.start()
